...
public class EmergencyDAO {
    ...
    private static final String SELECT_ALL_EMERGENCY_BY_BLOODGROUP = "SELECT * FROM emergency_requirement e, user_bloodbank b " +
            "WHERE b.code=e.bloodbank_code AND e.Blood_Group=?";
    ...
    public List<EmergencyBean> selectAllEmergencyByBloodGroup(String b_group){
        List <EmergencyBean > emergency = new ArrayList<>();
        try (PreparedStatement preparedStatement = con.prepareStatement(SELECT_ALL_EMERGENCY_BY_BLOODGROUP);) {
            preparedStatement.setString(1, b_group);
            ResultSet rs = preparedStatement.executeQuery();
